...
function EEex_GetImmediateLength(immediate)

	local absoluteImmediate = bit32.band(immediate, 0x80000000) ~= 0x0
		and bit32.bnot(immediate) + 1
		or immediate + 1
...
function EEex_EncodeGetImmediateLength(immediate)
...
--]]
function EEex_EncodeRM(args, func)
	...
	if argsReg then
		if opcodeExtension then
			EEex_Error("opcodeExtension and reg can't both be defined")
		end
		regOrExtension = EEex_RegToOrdinal[argsReg]
		if not regOrExtension then
			regOrExtension = EEex_RegWordToOrdinal[argsReg]
			if not regOrExtension then EEex_Error("Invalid reg") end
			prefix = 0x66
		end
	end
	-- Fill regOrExtension End

	local simpleReg = EEex_RegToOrdinal[argsRM]
	local simpleWordReg = EEex_RegWordToOrdinal[argsRM]

	if simpleReg or simpleWordReg then
		-- Direct reg, simple encoding
		if simpleWordReg then
			if argsReg and not prefix then EEex_Error("reg and rm must agree on operand size") end
			prefix = 0x66
		end
		...
		if argsRMLen < 3 or (argsRM:sub(1, 1) ~= "[" or argsRM:sub(argsRMLen) ~= "]") then
			EEex_Error("Invalid RM: \""..argsRM.."\"")
		end

		local plusMinusSplit, matchedPatterns = EEex_Split(argsRM:sub(2, argsRMLen - 1), "[%-+]", true, true)
		...
		local tryAsReg = function(section)
			local asReg = EEex_RegToOrdinal[section]
			if asReg then
				if rm then EEex_Error("Only one reg base allowed") end
				if sibByte then EEex_Error("Reg base must be defined before SIB") end
				rm = asReg
				return true
			end
			return false
		...
		local doAsSIB = function(baseReg, sibReg, scaleOrdinal)
			...
			if not sibReg then EEex_Error("Invalid RM") end
			if not scaleOrdinal then EEex_Error("Invalid RM") end
			if sibByte then EEex_Error("Only one SIB allowed") end
			...
			sibByte = EEex_Flags({
				baseReg,
				bit32.lshift(sibReg, 3),
				bit32.lshift(scaleOrdinal, 6),
			})
		...
		local tryAsSIB = function(section)
			...
			local asteriskSplit = EEex_Split(section, "*", false, true)
			local asteriskSplitLen = #asteriskSplit

			if asteriskSplitLen == 1 then return false end
			if asteriskSplitLen ~= 2 then EEex_Error("Invalid RM") end

			local sibReg = EEex_RegToOrdinal[asteriskSplit[1]]
			if not sibReg then EEex_Error("Invalid RM") end
			if sibReg == 4 then EEex_Error("ESP cannot be scaled") end
		...
		local doAsOffset = function(offsetIn, tryIndex)
			...
			if didAsOffset then EEex_Error("Only one offset allowed") end
			didAsOffset = true
	...
	if offset then
		EEex_ProcessNumberAsBytes(offset, mod == 1 and 1 or 4, func)
	end

	if immediate then
		local effectiveLength = (opcodeImmediateLength == 4 and explicitRMIndirectLength == 2) and 2 or opcodeImmediateLength
		EEex_ProcessNumberAsBytes(immediate, effectiveLength, func)
	end
end

function EEex_EncodeRMOpcode(args, encodingArgs)

	local firstArg = args[1]
	local secondArg = args[2]
	local firstOrdinal = EEex_RegToOrdinal[firstArg]
	local firstWordOrdinal = EEex_RegWordToOrdinal[firstArg]

	local opcode = encodingArgs.opcode
	if not opcode then EEex_Error("opcode must be defined") end
	...
	local attemptAsImmediate = function(arg)
		...
		if numAttempt then
			...
			if not immediateOpcodes then EEex_Error("immediateOpcodes must be defined when using immediate") end

			local foundImmediateOpcodeDef = nil
			local immediateLength = EEex_EncodeGetImmediateLength(numAttempt)
			...
			if not foundImmediateOpcodeDef then
				EEex_Error("immediateOpcodes.imm8 or immediateOpcodes.imm16/32 must be defined when using the corresponding immediate length")
			end
	...
	local toReturn = {}
	EEex_EncodeRM(encodingArgs, function(byte)
...
function EEex_GetOffsetAdjustment(state)
...
EEex_GlobalAssemblyMacros = {
	...
	["cmp_esi_edi"] = "3B F7",
	["cmp_esi_edx"] = "3B F2",
	["cmp_word:[ecx+dword]_byte"] = "66 83 B9",
	["dec_[ebp+byte]"] = "FF 4D",
	["dec_eax"] = "48",
	...
	["mov_byte:[esp+byte]_byte"] = "C6 44 24",
	...
	["restore_stack_frame_all"] = "5F 5E 5A 59 5B 58 8B E5 5D",
	["ret"] = "C3",
	["ret_word"] = "C2",
	["sar_eax"] = "C1 F8",
	["sar_edx"] = "C1 FA",
	...
	["test_al_al"] = "84 C0",
	["test_byte:[edi+byte]_byte"] = "F6 47",
	["test_eax_dword"] = "A9",
	["test_eax_eax"] = "85 C0",
	...
	["push"] = {
		["unroll"] = function(state, args)
			...
			local firstArg = args[1]
			local toPushOrdinal = EEex_RegToOrdinal[firstArg]
			...
			if numAttempt then
				if EEex_GetImmediateLength(numAttempt) == 1 then
					return {{0x6A, 1}, {numAttempt, 1}}
				else
					local toReturn = {{0x68, 1}}
					local i = 2
					EEex_ProcessNumberAsBytes(numAttempt, 4, function(byte)
						toReturn[i] = {byte, 1}
						i = i + 1
					end)
					return toReturn
				...
				return EEex_EncodeRMOpcode(args, {
					["opcode"] = 0xFF,
					["opcodeExtension"] = 6,
					["offsetAdjustment"] = EEex_GetOffsetAdjustment(state),
				})
			end
		end,
	},
	...
	["pop"] = {
		["unroll"] = function(state, args)
			...
			local toPushOrdinal = EEex_RegToOrdinal[args[1]]
	...
	["ret"] = {
		["unroll"] = function(state, args)
			...
			if EEex_GetImmediateLength(amount) <= 2 then
				local toReturn = {{0xC2, 1}}
				local i = 2
				EEex_ProcessNumberAsBytes(amount, 2, function(byte)
					toReturn[i] = {byte, 1}
					i = i + 1
				end)
				return toReturn
			else
				EEex_Error("!ret() cannot return > 0xFFFF")
			end
		end,
	},
